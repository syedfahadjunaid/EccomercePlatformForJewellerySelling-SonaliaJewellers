...
function ProductPage() {
  ...
  useEffect(() => {
    setCount((product?.length / 10).toFixed(0));
  }, [product]);
  const getProductHandle = () => {
    const filter = product.filter((item) => item.productSubCategory === Id);
    setAllProduct(filter && filter.reverse());
    console.log(filter);
  };
  const getProductPriceFilter = async () => {
    const filter = allProduct?.filter(
      (item) => item?.gold?.carat === productKarat
    );
    console.log(filter, "filter product");
  };
  useEffect(() => {
  ...
  useEffect(() => {
    console.warn(Id, "fvfg g ff gcfd sdf");
  }, [Id]);
  ...
  const filterHandle = () => {
    ...
    if (selectedKarat !== "") {
      ...
      const filter = selectedValue?.filter((item) => {
        ...
        if (item?.gold?.carat === 14) {
          goldPrice =
            Math.floor(((price.slice(-1)[0]?.GoldPrice / 100) * 59.2) / 10) *
            item?.gold?.weight;
        ...
        console.log(total, "settotal fgf", goldPriceShow);
        const final = total <= Number(productRate);
        {
          final === true && final && arr.push(item);
        }
      ...
      <div className="productpage">
        ...
        <div className="productpage_main_section">
          ...
          <div className="productpage_main_section_right">
            <div className="productpage_main_section_right_top"></div>
